...
@doc raw"
Create a discrete probability cumulative distribution function (CDF) from a given uncensored delay distribution,
assuming a distribution over primary event times with censoring intervals of width `Δd`.
...
```
"
function censored_cdf(dist::PrimaryCensoredDist; Δd = 1.0, D = nothing, upper = 0.999)
